...
#
# libdiagnose ships as -betterC, which has no druntime, while D's -cov registers
...
# supplies a druntime `main` when `DiagnoseCoverage` is set.
#
...
# dub lists only libdiagnose's own modules; libfp has to be compiled in too,
# since -I alone would leave its symbols undefined at link time. The paths
# left absolute above are exactly the ones outside the project.
...
# ran.
"$compiler" -cov -g -unittest "${versionFlag}DiagnoseCoverage" \
	"${importPaths[@]/#/-I}" "${sources[@]}" -of="$out/diagnose-coverage$exeSuffix"
...
"./$out/diagnose-coverage$exeSuffix" --DRT-testmode=run-main \
...
# everything that is not libdiagnose's own -- libfp gets measured by its
